...
module.exports = function (index, storage, app, done) {
    ...
    mongoose.connect(storage.server || 'localhost', storage.database || 'test');
    ...
    db.on('open', function callback () {
        ...
        app.Storage.mongodb = function (config) {

            // Schema
            if (!schema) {
                schema = new mongoose.Schema(config.schema);
            }

            // Model
            if (!this.Model) {
                this.Model = mongoose.model(config.name, schema);
            }
